...
learning_rate = 0.001
epsilon = 12        # Define a threshold for acceptable error
...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Load the current and last 9 control values (or less if at the beginning of the sequence)
        ...
        if len(past_controls) < sequence_length:
            padding = np.zeros((sequence_length - len(past_controls), 2), dtype=np.float32)
...
def load_data():
    ...
    control_df = pd.read_csv('DRIVE/Data/data1/control_data.csv')

    # Extract control data (speed and angle) and image paths
    control_data = control_df[['speed', 'angle']].values
    image_paths = control_df['dir'].apply(lambda x: os.path.join('DRIVE/Data/processed_images_1', x)).tolist()
...
class ConvNet(nn.Module):
    ...
    def forward(self, x, control_seq):
        ...
        combined_out = self.tanh(combined_out) * 100  # Scale the output to the range [-100, 100]
...
def train_model(model, train_loader, test_loader):
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_loader:
            ...
            loss = criterion(outputs, control_seq[:, -1, :])
            ...
            absolute_errors = torch.abs(outputs - control_seq[:, -1, :])
        ...
        if (epoch + 1) % 5 == 0:
            ...
            with torch.no_grad():
                ...
                for batch in test_loader:
                    ...
                    outputs = model(images, control_seq)
                    test_loss += criterion(outputs, control_seq[:, -1, :]).item()
                ...
                print(f'Test after Epoch {epoch+1}: Loss: {avg_test_loss:.4f}, Accuracy: {test_accuracy:.2f}%')

                # Check for early stopping
                if test_accuracy > best_accuracy:
                    best_accuracy = test_accuracy
                    no_improvement_count = 0
                    torch.save(model.state_dict(), 'DRIVE/model_with_control_seq.pth')  # Save the best model
    ...
    if no_improvement_count < early_stopping_threshold:
        torch.save(model.state_dict(), 'DRIVE/model_with_control_seq.pth')
    else:
        print('Training stopped early, model saved as model_with_control_seq.pth.')
