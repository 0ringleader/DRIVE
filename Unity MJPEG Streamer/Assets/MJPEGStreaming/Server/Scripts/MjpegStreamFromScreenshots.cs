...
public class MjpegStreamFromScreenshots : MonoBehaviour
{
    public Camera cameraToCapture;
    public int port = 8000;
    public float captureInterval = 0.1f; // Interval between screenshots in seconds
    ...
    void OnContextReceived(IAsyncResult result)
    {
        ...
        Debug.Log("Client connected for MJPEG stream");
    ...
    IEnumerator StreamFrames(HttpListenerResponse response)
    {
        while (isStreaming)
        {
            ...
            try
            {
                ...
                Debug.Log("Frame sent to client");
            }
            catch (Exception ex)
            {
        ...
        Debug.Log("Client disconnected from MJPEG stream");
    }
