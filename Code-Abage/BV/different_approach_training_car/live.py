...
# Model parameters (match these with your trained model)
input_height = 192
input_width = 256
num_channels = 1  # Grayscale
num_classes = 1  # Only predicting angle
sequence_length = 10
...
# Model definition (reuse the architecture from your training script)
class ConvNet(nn.Module):
    def __init__(self, num_classes, sequence_length):
        ...
        self.control_fc = nn.Linear(sequence_length, 64)  # Match the original shape
...
model.load_state_dict(torch.load('cnn9_pro_data_model_with_control_seq_test.pth'))
model.eval()

# Contains the past control values
past_controls = [[0]] * sequence_length  # Initialize with zeros
...
def preprocess_frame(frame):
    ...
    pil_image.save("preprocessed_frame.png")
...
def process_image_and_control(image):
    ...
    print(f"past_controls_tensor shape: {past_controls_tensor.shape}")
    ...
    if len(past_controls) >= sequence_length:
        past_controls.pop(0)
    past_controls.append([steering_angle])

    # Debug information
    print(f"inputs to model: {processed_frame.shape}")
    print(f"outputs from model: {control_values}")
...
try:
    ...
    while running:
        ...
        if frame is not None:
            ...
            car_control.setControlData(float(30), float(steering_angle))
