...
public class HttpVideoStream : MonoBehaviour
{
    public int port = 8000; // Port, über den der Stream bereitgestellt wird
        ...
        while (isStreaming)
        {
            using (TcpClient client = listener.AcceptTcpClient())
            {
                using (NetworkStream stream = client.GetStream())
                {
                    ...
                    string requestString = Encoding.ASCII.GetString(requestBuffer, 0, bytesRead);
    ...
    void Update()
    {
        ...
        {
            Debug.LogError("Streaming camera or target texture is not set.");
            return;
        }
